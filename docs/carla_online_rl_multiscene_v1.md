...
该协议本身不是训练完成证明。代理训练、静态校验和 dry-run 不能替代真实 CARLA；只有服务器上的真实运行结果才能支持风险、碰撞、传感器和路线结论。当前冻结 dev/test 的实机结果记录在本文末尾。
...
## 冻结 dev/test 实机结果（2026-09-02）

最终模型使用固定计划 `seed=20260824`，独立评估种子为 `20360824`。dev 和 test 均在 CARLA `0.9.16` 上完成，任务退出码均为 `0`：

| split | 作业 | 场景 | 候选执行 | 有效候选 | 四项验收 | 约束投影 |
|---|---|---:|---:|---:|---:|---:|
| dev | `carla-rl-04-evaluate-dev-v1_20260902_115107` | 27 | 432/432 严格通过 | 432/432 | 4/4 | 14 |
| test | `carla-rl-05-evaluate-test-v1_20260902_160817` | 24 | 384/384 严格通过 | 384/384 | 4/4 | 14 |

test 的四项门均为 `24/24`：baseline 严格验收、候选条件有效性、候选运行严格验收和候选证据完整性全部通过；缺失 `run_dir`、错误风险方法和 CARLA 服务异常均为 `0`。这些结果证明冻结模型可在独立 test 集上稳定产生合法、可运行、可审计的候选。

策略效果必须与工程验收分开解释。test 上 baseline 平均风险分为 `57.845`，第 16 步最终候选平均风险分为 `48.653`，平均变化 `-9.193`，中位变化 `-6.306`；风险上升 `9/24`，下降 `15/24`。按来源生成器分组，GMM 为 `+0.698`（上升 `4/8`），CVAE 为 `-7.987`（上升 `3/8`），LHS 为 `-20.289`（上升 `2/8`）。按目标风险档分组，low/medium 分别为 `+8.488/+9.403`，high/critical 分别为 `-20.702/-11.762`。这些都是固定小样本上的描述性结果，不做显著性或跨地图外推。

因此，SAC `10,000` 步实验的结论是：多场景在线训练、断点恢复和独立 dev/test 验收链路已经完成，但策略未在冻结 test 上证明总体对抗性风险提升，尤其没有证明 high/critical 场景泛化。后续材料可以报告该负向结果，不得写成“SAC 普遍优于基线”或“已实现跨地图泛化”。

本地轻量证据：

- dev：`F:\Carla\project-transfer\server-results\carla-rl-04-evaluate-dev-v1_20260902_115107_final\test_evaluation_summary.json`，SHA-256 `A3FAFEAABD8BDB2D354196C1C2FB0D535314518AF1962FE0A415D3E7D8A41C6B`。
- test：`F:\Carla\project-transfer\server-results\carla-rl-05-evaluate-test-v1_20260902_160817_final\test_evaluation_summary.json`，SHA-256 `33C90DE4F9ADBCBC966380EFE3640244FDFFD9F245144FCB91708A0E4FB6E097`。

评估完成后，项目管理的 CARLA tmux 会话已停止，RPC `2000` 已关闭，GPU1 项目锁已释放；独立 TensorRT 服务未被停止。
