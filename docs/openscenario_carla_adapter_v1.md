...
| `pedestrian.trigger_seconds` | 行人事件的仿真时间触发器 | 使用标准 `LongitudinalAction/SpeedAction` 近似行人启动 |
| `ego_vehicle.blueprint` / `lead_vehicle.blueprint` | `Vehicle/@name` | 写入真实 CARLA blueprint，逻辑实体名仍由 `ScenarioObject/@name` 保留 |
| `scenario.ego_spawn_index` | `ego` 的 `WorldPosition` + `carla:ego_spawn_index` | 位姿由目标地图查询后通过 `--ego-world-position` 显式绑定 |
...
1. 仅凭 `ego_spawn_index` 跨地图自动解析真实坐标和车道拓扑。
...
6. 将最小 XOSC 直执行结果自动转化为 Scene 04 的多传感器风险验收。

当前 P2 已在服务器完成一条最小 XOSC 的 ScenarioRunner 直执行；这只证明实体生成、Storyboard 触发和清理可运行，不扩展为完整 CARLA 语义兼容。

### 为什么暂不支持

| 边界 | 当前原因 | 当前替代方式 |
| --- | --- | --- |
| 真实地图坐标绑定 | 生成记录只有 `ego_spawn_index` 和相对距离，没有跨地图通用的 `x/y/z`、道路 ID 或车道 ID。 | 由目标 CARLA 地图查询 spawn point，再通过 `--ego-world-position X Y Z H P R` 写入 XOSC；本次使用 Town10HD_Opt index 40。 |
...
| ScenarioRunner 实际执行兼容 | 已完成一条最小单场景实机冒烟，但纯 XOSC 没有 ScenarioRunner criteria，也不承载项目传感器/风险验收。 | 运行日志、版本、地图、TM 端口和 XOSC 哈希单独登记；完整风险结果仍由 Scene 04 旁路链路提供。 |
...
python tools\convert_scenario_to_openscenario.py `
  ...
  --ego-world-position X Y Z H P R
```
...
`--ego-world-position` 为可选的六元组 `X Y Z H P R`；不提供时保留零坐标交换占位，只适用于静态校验。要做 ScenarioRunner 直执行，必须先在目标 CARLA 地图查询 spawn point 或 waypoint，再传入该位姿。
...
静态通过只证明：输入记录合法、映射配置合法、XML 可解析且包含实体/事件/停止触发器、CARLA JSON 具备 Scene 04 的顶层结构。它不证明 CARLA 服务在线、地图坐标有效、传感器能写盘、路线能完成或风险指标可计算。
