...
Web MVP 是阶段五的首个交付增量。它把现有 M07 只读 Dashboard 统一成一个可访问的本地 Web 入口，先展示场景库和运行证据，再逐步接入生成、校验、任务编排和风险分析。当前版本不启动 CARLA、不写入场景库、不占用 GPU。
...
| `/generation`、`/validation`、`/tasks`、`/risk` | 边界预留 | 后续模块入口，当前只读占位页 |
...
当前验收已完成桌面截图和 Playwright `390×844` 移动视口检查；移动视口下页面文档宽度等于 `390`，导航分为两行，117 条场景数据加载完整，无横向页面溢出。Web 阶段基线为 `101` 项；加入 Diffusion 对照单测后全量 Python 测试为 `103` 项通过、1 项因本机未安装可选 Stable-Baselines3 跳过。
