...
| 当前工程基线 | `4b02a78`，阶段五 M08 最小演示链路（本轮整理前基线） | 功能完成后记录并冻结最终 V1.0 提交 |
| 当前运行基线 | CARLA 0.9.16、`Carla666-0916` | 冻结版本后重新做一次环境和版本核对 |
| 当前软件形态 | Python 命令行、JSON/CSV 文件接口、本地只读 Dashboard | 说明书按实际交付形态描述，不虚构多用户产品 |
...
| M02 约束校验 | `core/scenario_validator.py`、`schemas/` | Schema、语义校验、配置编译、错误路径测试 | 已验证实现 | 固定说明书中的正常流程和一个负例 |
...
| M07 只读 Dashboard | `tools/scenario_dashboard.py`、`tools/scenario_dashboard.cmd` | 页面级回归、HTTP 接口、场景库展示 | 原型能力 | 冻结版本采集页面截图；不宣称多用户、权限和写入能力 |
...
| 检查 | 当前结果 | 证据边界 |
|---|---|---|
| 全量单元测试 | `92` 项通过，`1` 项按预期跳过 | 跳过项依赖可选 Stable-Baselines3；不影响 M01–M08 基础链路，不能据此证明训练能力 |
...
| S04 | 场景库筛选和详情 | `tools/scenario_dashboard.cmd` | 待冻结后采集 | 页面只读，显示数据快照日期 |
