...
|---|---|---|---|---|
| M01 场景生成 | `tools/generate_seed_dataset.py`、`tools/generate_with_model.py`、`models/`、`training/` | 生成记录、Schema、模型选型报告、离线生成测试 | 已验证实现 / 研究原型 | 选定最终展示命令和一组可复现输入输出 |
...
| 检查 | 当前结果 | 证据边界 |
|---|---|---|
| 全量单元测试 | `101` 项通过，`1` 项按预期跳过 | 跳过项依赖可选 Stable-Baselines3；Gymnasium 缺失断言按设计执行，不影响 M01–M08 基础链路，不能据此证明训练能力 |
