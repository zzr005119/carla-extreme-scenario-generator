...
# ==================== 7. 行人（路边等待 → 第3秒突然横穿） ====================
# 行人出生在道路右侧前方 ~30m 处（路边）
road_right = 8.0  # 偏移到主车右侧路肩
walker_bp = bp_lib.filter("walker.pedestrian.0007")[0]
walker_start = carla.Location(
    x=ego_sp.location.x + 30 * math.cos(yaw_rad) - road_right * math.sin(yaw_rad),
    y=ego_sp.location.y + 30 * math.sin(yaw_rad) + road_right * math.cos(yaw_rad),
    z=ego_sp.location.z
)
...
if walker:
    ...
    walker_ctrl.start()
    walker_ctrl.stop()
    print(f"[WALKER] 行人已在路边等待 ({(walker_start.x:.1f)}, {walker_start.y:.1f})")
else:
    print("[WALKER] 失败")
...
try:
    for step in range(20):
        ...
        if t == 3 and walker_ctrl and not walker_triggered:
            ...
            walker_ctrl.set_max_speed(3.5)  # 快速冲出 (3.5 m/s ≈ 12.6 km/h)
