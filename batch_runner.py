...
def run_batch(batch_path, batch_config, base_config_path, base_config, limit=None):
    ...
    for index, variant in enumerate(variants, start=1):
        ...
        config_path = os.path.join(generated_dir, f"{variant_name}.json")
        log_path = os.path.join(logs_dir, f"{variant_name}.log")
        with open(config_path, "w", encoding="utf-8") as file:
            json.dump(variant_config, file, ensure_ascii=False, indent=2)

        print(f"[BATCH {index}/{len(variants)}] 开始: {variant_name}")
        environment = os.environ.copy()
        environment["PYTHONIOENCODING"] = "utf-8"
        output_dir = None
        lines = []
        ...
        print(
            f"[BATCH {index}/{len(variants)}] 结束: "
            f"{row['status']} / 风险={row['risk_level'] or 'unknown'}"
        )

        if process.returncode != 0 and not batch_config.get("continue_on_error", True):
            break
    ...
    return 0 if batch_metadata["failed_count"] == 0 else 1
...
def main():
    ...
    if args.validate_only:
        print(f"[BATCH] 配置校验通过，共 {len(variants)} 个变体")
        return 0
