...
3. ▶ **S5-RL-01 CARLA 在线 RL**：修复后的 SAC `256` 步 canary 已在服务器 GPU1 完成，质量门 `15/15` 通过，`265/265` 条 CARLA 执行严格通过，模型与 checkpoint 已保存于独立 retry 目录。主训练已由阶段 03 从 `2,000` 步 checkpoint 完成至 `10,000/10,000` 步，10 个 checkpoint 和最终模型均存在；作业 `carla-rl-03-resume-sac-10000-v1_20260828_112459` 的训练清单为 `completed`。全量历史审计保留为 `14/15`，唯一失败 `all_executions_strict` 来自恢复前旧 episode 的 `11` 条记录；恢复后的新 episode 共 `8,477/8,477` 条严格通过，旧失败记录已在独立新版回归中 `11/11` 通过且未覆盖原证据。质量门已新增按 episode/证据版本分层的审计，当前 episode 独立门为 `16/16`（审计作业 `audit-sac-10000-current-episode-v1_20260831_113919`），因此无需重跑训练；修复后的 dev 作业 `carla-rl-04-evaluate-dev-v1_20260902_115107` 已完成 `27/27`，四项独立验收门 `4/4` 通过并以退出码 `0` 结束；冻结 test 作业 `carla-rl-05-evaluate-test-v1_20260902_160817` 已启动，完成验收前仍不据此宣称 RL 泛化。协议见 `docs/carla_online_rl_multiscene_v1.md`。
...
- 多场景 RL 泛化准备 V1 已完成：`core/carla_rl_plan.py` 和 `tools/prepare_carla_rl_multiscene_plan.py` 将场景库固定为 `train/dev/test=66/27/24`，按 12 个生成器×风险分层分配，并检查 canonical ID/scenario hash 泄漏；`tools/train_carla_rl.py` 保留单记录冒烟兼容，同时支持 SAC/PPO、多场景 sampler、`1,000` 步 checkpoint、`.zip` resume、run/checkpoint manifest；`tools/evaluate_carla_rl_multiscene.py` 支持冻结 dev/test split。服务器操作已拆为 00 准备、01 SAC 256 canary、02 SAC 10,000 主训练、03 断点恢复、04 dev 评估、05 test 验收六个独立入口；canary 已由 `tools/check_carla_rl_training.py` 验收通过，主训练已完成 `10,000` 步；全量历史门保留 `14/15`，当前 episode 门已验证 `16/16`。独立评估固定 baseline 严格、候选条件有效、候选运行严格、候选证据完整四项验收口径；首轮修复版 dev 作业 `carla-rl-04-evaluate-dev-v1_20260831_184326` 的 `27/27` 运行均严格通过，但 Gym 适配层漏传 `run_dir`，导致证据门汇总为 `0/27` 并以 `EXIT_CODE=1` 结束。提交 `569eb6d` 已修复；retry `carla-rl-04-evaluate-dev-v1_20260902_111116` 在服务器重启后中断，随后作业 `carla-rl-04-evaluate-dev-v1_20260902_115107` 已在 CARLA 0.9.16/GPU1 完成 dev split `27/27`，四项独立验收门 `4/4` 通过并以退出码 `0` 结束；CARLA 会话已停止，冻结 test 尚未启动。说明见 `docs/carla_online_rl_multiscene_v1.md`。
- Web 产品化首期 P0 已完成：统一入口 `tools/web_app.py`/`tools/web_app.cmd` 复用场景库 API，提供 Dashboard、场景列表、详情、受控查询、健康检查，以及生成/校验/风险分析三条可操作表单流程；提交后由 CPU worker 执行，页面轮询任务状态并展示成功产物、结构化失败和取消结果。已修复任务页内联 JavaScript 转义错误，并用 Edge 无界面浏览器确认 `/api/tasks` 结果可实际渲染到任务表；任务结果列已固定宽度并单行省略，完整内容通过悬浮提示查看。校验支持 JSON/JSONL、物理约束和可选 CARLA 配置编译；CARLA 任务显式确认或取消后仍转交外部入口，不由 Web 启动。最新项目环境全量回归为 `163 passed / 5 skipped`（1 项可选 SB3 依赖、4 项 MJX-JAX 测试默认关闭），`compileall` 和冻结内容门均通过；真实 HTTP 冒烟覆盖页面 `200`、三类任务完成、CARLA 取消和 `carla_connected=false`。说明见 `docs/stage5_web_product_flow_v2.md`。
- RL 独立评估口径修复已落地：`candidate_constraints.project_requested_weather` 对动作造成的缺失天气标签执行有界、可审计投影，保留原始违规计数；`test_evaluation_summary.json.acceptance` 固定四项独立门，结构/运行失败仍不会被放宽。提交 `569eb6d` 另修复 Gym 适配层的候选 `run_dir` 证据透传；`carla-rl-04-evaluate-dev-v1_20260902_115107` 已完成并确认四项门通过，第 05 步 test 作业 `carla-rl-05-evaluate-test-v1_20260902_160817` 正在运行。
...
- `S5-PYBULLET-02` 接触稳定性筛选已完成：8 组平衡参数组合在 CPU/GPU1 均 `8/8` 通过，最大 CPU/GPU 间距差 `8.88e-16 m`，最小真实间距 `3.393202 m`；代表性 `iterations=4` 候选的自定义 VJP 与 `jacfwd` 最大差均为 `0`。独立相向运动边界扫描又在服务器 CPU/GPU1 各完成 `6` 组：两侧均 `6/6` 数值门通过，`4` 组发生接触，接触门均为 `3/4`，最大穿透约 `0.080471 m`，CPU/GPU1 最大间距差 `9.77e-15 m`、最大损失差 `7.02e-14`、6 组分类完全一致；整体物理真实性门仍为 `false`。GPU1 任务已正常结束并释放锁，未修改 GPU0 的 vLLM/TensorRT 服务。该结果只证明受控小场景的数值边界，不证明车辆动力学真实性；暂不进入 CVAE/Diffusion 长时序训练。证据见 `artifacts/p4_1_mjx_differentiable_poc_v1/server_cpu_contact_boundary_v1/manifest.json`、`server_gpu1_contact_boundary_v1/manifest.json` 和 `manifest_server_cpu_gpu1_contact_boundary_comparison_v1.json`。
- `S5-PYBULLET-02` 接触稳定性参数优化复核已完成（CPU/GPU1，且不覆盖原始压力边界）：保持约 `2.56 s` 总物理时长时，推荐 `timestep=0.00625, horizon=410, iterations=4` 在两设备均为 `6/6` 数值门、`4/4` 接触门，最大穿透约 `0.033941 m`；6 组分类完全一致，最大最小间距差 `8.88e-16 m`、最大损失差 `1.78e-15`、最大解析梯度差 `1.11e-15`。最高力案例自定义 VJP 与 `jacfwd` 最大差在两设备均为 `0`，有限差分相对误差为 CPU `2.54e-4`、GPU1 `3.33e-4`；GPU1 与原生 MuJoCo 最大位置误差 `1.17e-11 m`。两次 GPU1 作业均正常退出并释放锁，GPU0 的 vLLM 未修改，结束后 GPU1 仅保留外部 TensorRT 服务。`timestep=0.005` 仍有 `0.050855 m` 穿透，说明接触改善不具单调性；该 profile 只能写成 toy 场景的受控稳定工作区间，不升级为车辆真实性。证据目录为 `F:\Carla\project-transfer\server-results\mjx_optimization_screen_20260902`，逐组对比见 `cpu_gpu1_optimization_comparison_v1.json`，详细记录见 `docs/p4_1_mjx_differentiable_poc_v1.md`。2026-09-02 实时复核确认 GPU1 空闲且无 MJX/CARLA 残留；已按汇总清单删除服务器上的 `contact_dt005_i16.json` 与 `contact_dt010_i8.json` 两份冗余副本，保留哈希和等价性记录。
...
3. Web 生成/校验/风险页面已经完成首期产品化，不再重复建设同一层原型；后续只补真实演示证据、权限/部署等明确需求。在线 RL 泛化和 PyBullet/MJX 研究闭环单独排期；当前 MJX 前向默认按规模选择服务器 CPU/GPU1，GPU1 优先用于 batch 自定义 VJP 等受益场景并受显存上限约束；原始双刚体压力边界仍保留，`timestep=0.00625, horizon=410, iterations=4` 已完成 CPU/GPU1 同配置复核并作为 toy 场景受控稳定 profile，但不把它升级为车辆真实性，`iterations=1` 不作为默认物理求解方案，自定义 VJP 暂不接入高维长时序训练。ScenarioRunner 已完成一条原生直执行和一条关联样本的 Scene 04 完整多传感器/路线/风险验收，跨地图与批量泛化仍不宣称。
4. 维护 `docs/stage5_material_index_v1.md` 中的一键演示 `demo_manifest.json` 路径、SHA-256、关键计数和重建命令；不把历史风险证据写成新 CARLA 实测。
5. 完整训练验收结束后 CARLA 已停止；GPU1 仅保留 root 管理的 TensorRT 服务，GPU0 的 vLLM 不修改。后续 GPU/CARLA 任务启动前重新检查服务和显存状态。
