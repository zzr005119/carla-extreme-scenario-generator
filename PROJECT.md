...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步/多步闭环、异常路径编排测试、Gymnasium 外壳、环境级 CARLA 冒烟、场景库分层采样、四类非学习基线、reward V2、60 次 CARLA 严格对照、Stable-Baselines3 训练工程链路、冻结 27 维风险代理执行器、多随机种子代理基准、SAC/rule-guided LHS 的 12 分层 CARLA 独立评估、3 个原始 pair × 3 个 Traffic Manager 种子的 `27/27` 重复复验、新增 6 个未覆盖分层的 `18/18` CARLA 严格验收、18 个独立 pair 的合并统计、质量门审计和 2 个优先分层的 `18/18` 重复测量均已完成；下一步复核 LHS/high 候选参数与风险代理排序边界，必要时设计新的独立场景，不启动 CARLA 在线训练。
...
## 下一步
- LHS/high 候选参数与代理排序边界复核 V1 已完成：对物理增强评分的 `256` 个 LHS/high 候选离线复核，源场景 `lhs_high_20260817_0223` 的稳健代理分 `58.866`、均值分 `61.956`、标准差 `6.181`，稳健分降序排名 `14/256`；运行侧基线代理 `55.032` 与物理增强分相差 `-3.834`。重复实测中 SAC/rule-guided LHS 的风险增量为 `+29.254/+34.077` 且均为 `3/3` 新增碰撞，明显大于代理增量 `+4.791/+8.456`，说明排序方向可参考但增量幅度和碰撞边界未校准。离线工具为 `tools/analyze_lhs_high_proxy_boundary.py`，3 个未在当前计划出现的静态候选为 `lhs_high_20260817_0203`、`lhs_high_20260817_0022`、`lhs_high_20260817_0041`；报告和参数差分位于 `F:\Carla\project-transfer\lhs_high_proxy_boundary_v1_20260822`。候选尚未运行 CARLA，不构成实测风险结论。
- 2026-08-22 GPU1 状态复核：服务器 CARLA `0.9.16` 进程 PID `754020` 正常运行并占用约 `6166 MiB`，RPC `2000` 可连接，`get_world()`、`Town10HD_Opt`、快照和客户端/服务端版本检查均正常；GPU1 另有 root 管理的 TensorRT `python3` PID `3841027` 占用约 `896 MiB`。两者均不是本项目可安全终止的死锁进程，未执行停止操作；GPU0 的 vLLM 继续保持不动。后续计划生成使用 `server_run.cmd -Resource Cpu`，不与 GPU1 并发抢占。
1. 保留质量门审计结论，按生成器、目标风险档和碰撞状态追踪后续结果；当前不把 high/critical 碰撞集中误判为基础设施质量门失败。
2. LHS/high 已完成 3 个新种子重复测量，下一步优先复核其候选参数和代理排序边界，不继续重复同一 pair；如需新增 CARLA 运行，应优先设计新的独立场景。
