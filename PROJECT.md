...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步/多步闭环、异常路径编排测试、Gymnasium 外壳、环境级 CARLA 冒烟、场景库分层采样、四类非学习基线、reward V2、60 次 CARLA 严格对照以及 Stable-Baselines3 低成本训练工程链路均已完成；下一步接入冻结的 27 维风险代理作为近似训练执行器，再设计与现有 12 分层和 reward V2 一致的少量 CARLA 独立评估，不直接启动 CARLA 在线训练。
...
## 下一步
1. 接入冻结的 27 维物理增强风险代理作为只读近似执行器，固定模型哈希、输入特征版本和训练数据边界；历史 `observed_risk` 只用于代理推理，不得回流为当前 episode 的伪实测结果。
2. 在近似执行器上进行多随机种子 SAC/PPO 低成本训练与稳定性检查，并以相同候选预算对照 fixed/random/LHS/rule-guided LHS；所得结论只属于代理环境。
3. 冻结少量 12 分层 CARLA 独立评估计划：策略推理后不在线更新，继续使用共享基线、严格验收和 reward V2；以 rule-guided LHS 为主要非学习对照，同时保留其余三类基线。
4. 只有少量独立评估证明策略候选可执行且具有实测增益后，才讨论扩大 CARLA 预算；继续维护场景库质量门和 Dashboard 回归，软著演示截图后置到正式申请准备阶段。
