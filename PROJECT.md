...
- V1.0 冻结前检查已建立：`tools/check_stage5_freeze.cmd` 覆盖材料入口、M01–M08 口径、M08 清单契约、117/351 场景库计数、产物哈希、Carla666-0916 环境和工作区状态；当前内容检查 `30 PASS / 0 FAIL`。`--require-clean` 目前仅因用户保留的未提交 `data/scenarios/seed_v1/example_record.json` 报 `git_clean`，不属于项目实现改动；正式冻结前需由用户决定恢复或单独处理该文件，正式提交、最终截图和申请主体信息仍为 PENDING。
...
- RL 独立评估口径修复已落地：`candidate_constraints.project_requested_weather` 对动作造成的缺失天气标签执行有界、可审计投影，保留原始违规计数；`test_evaluation_summary.json.acceptance` 固定四项独立门，结构/运行失败仍不会被放宽。提交 `569eb6d` 另修复 Gym 适配层的候选 `run_dir` 证据透传；当前 retry dev 完成并确认四项门通过后，才允许第 05 步 test。
