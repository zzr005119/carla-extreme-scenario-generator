...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配和对抗性测试代理 V1 契约已完成，下一步接入闭环实验编排与 CARLA 实测反馈；当前尚未训练 RL 模型。
...
## 下一步
1. 建立“生成场景 → 代理候选 → CARLA 严格执行 → 风险回填 → 下一候选”的闭环实验编排接口。
2. 先用固定动作策略完成单 episode 离线/实机冒烟，验证失败恢复、结果追踪和终止条件，再决定是否引入 `gymnasium` 与 Stable-Baselines3。
