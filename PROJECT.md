...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理、误差/排序稳定性诊断和反馈候选评分 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，27 个反馈短名单的 81 次外部验证已严格验收 81/81。63 个独立场景的风险反馈数据集 V2、风险代理复训、50 次重复 OOF、同口径 Top-9 对比、独立碰撞倾向诊断和双通道离线候选评分均已完成；当前进入双通道候选的服务器冒烟与小规模配对实机验证。**
...
阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条唯一 CVAE 样本实测记录、完整受控重复性验证、三生成器 108 次严格验收、第一轮 36 个独立场景风险反馈聚合、第一版风险代理基线、1536 条反馈候选的三通道统一评分，以及 27 个新增场景的 81 次外部验证；当前双通道评分离线比较和 54 次配对验证计划已完成，实机验证尚未开始。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测、第一版风险反馈诊断和三通道候选评分已完成；受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，反馈短名单外部验证严格验收 81/81，风险反馈与代理 V2、双通道离线评分已完成，当前验证双通道候选是否能在 CARLA 中提高碰撞边界覆盖。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次实机严格验收；✅ 三生成器 108 次仿真、严格验收与阶段结论冻结；✅ 风险反馈数据集 V1 与代理基线；✅ 代理误差和排序稳定性诊断 V1；✅ 反馈候选 81 次外部验证；✅ 风险反馈数据集、风险代理和碰撞通道诊断 V2；✅ 双通道离线评分与 54 次配对计划；▶ 双通道服务器冒烟和配对实机验证，不进行最终 Demo 封版。
...
## 下一步
1. 将当前已提交版本推送到 GitHub，并通过 `tools\server_sync.cmd` 同步服务器运行工作区。
2. 启动服务器 CARLA，先执行双通道验证计划中的 `run_smoke.sh`，确认 GPU 1、CARLA RPC、地图和单次配置链路正常。
3. 冒烟通过后，再运行完整 `54` 次配对验证，并用收集工具生成严格验收、碰撞覆盖和通道对比报告。
4. 根据实测结果决定继续碰撞边界主动补样，还是进入潜空间条件 Flow 对照；在 54 次结果出来前不进行最终 Demo 封版。
