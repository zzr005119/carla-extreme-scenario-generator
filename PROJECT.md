...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理、误差/排序稳定性诊断和反馈候选评分 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，27 个反馈短名单的 81 次外部验证计划已完成静态验收，服务器首轮已产生 4 次有效结果，当前准备按碰撞感知路线口径恢复剩余批次。**
...
阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条唯一 CVAE 样本实测记录、完整受控重复性验证、三生成器 108 次严格验收、36 个独立场景风险反馈聚合、第一版风险代理基线和 1536 条反馈候选的三通道统一评分。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测、第一版风险反馈诊断和三通道候选评分已完成；12 场景 × 3 种子的受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，当前验证稳健高分、高不确定性和碰撞边界主动补样短名单。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次实机严格验收；✅ 三生成器 108 次仿真、严格验收与阶段结论冻结；✅ 风险反馈数据集 V1 与代理基线；✅ 代理误差和排序稳定性诊断 V1；▶ 反馈引导候选排序与外部验证，不进行最终 Demo 封版。
...
- 反馈短名单外部验证 V1 已完成准备与静态验收：27 个独立场景各覆盖三个 Traffic Manager 种子，共 `81/81` 份配置通过 `--validate-only`；调度为 9 个随机化区组、27 个三场景小批次，生成器 × 选择通道的 9 个单元格各包含 3 个独立场景。服务器批次已开始执行。
- 服务器首轮外部验证已产生 4 次完整运行。前三次通过原全程路线验收；第 4 个 CVAE 临界候选在两次相同配置运行中均于行人碰撞后触发全程路线失败，其中首次运行的碰撞前双车同时在途率为 `1.0`、主车最大偏差为 `0.992 m`，全程最大偏差为 `5.55 m`。因此本实验采用结果感知口径：无碰撞时继续要求全程路线合格；发生碰撞时只用首次碰撞前遥测判断场景执行有效性，碰撞后的全程路线数据仍保留为结果证据。若首次碰撞前已偏离路线，仍严格判为失败。
- 本轮唯一的初次路线失败在相同配置重跑后恢复为双车全程在途、路线偏差小于 `1.01 m`；当前按偶发运行状态处理，但后续批次仍保留路线严格验收，不能删除该质量门槛。
- 按原计划，2026年7—8月应处于阶段三；截至2026年8月15日，阶段二已形成第一版可验证模型基线、受控重复性证据、三生成器 108/108 严格实测对照、风险代理基线、重复 OOF 误差诊断和反馈短名单，CARLA 0.9.16 三种子迁移回归已通过，当前进入 27 个短名单的外部验证准备。
...
## 下一步
1. 同步碰撞感知路线验收修复，在原运行目录上生成新版清单并复用 4 次已有结果，由可恢复运行器继续剩余 77 次。
2. 完成全部 81 次仿真；非碰撞运行检查全程路线，碰撞运行检查首次碰撞前路线，同时继续检查传感器完整性、CARLA 服务健康和风险元数据。
3. 聚合 27 个新增独立场景，分别评估稳健分与实测风险的排序误差、三个通道的碰撞/高风险发现率，以及新增样本加入后对风险代理误差和 Top-K 稳定性的修正作用。
4. 将验收通过的新增场景并入反馈数据集，复训风险代理并复查误差与 Top-K 稳定性；达到样本门槛后再决定是否训练独立碰撞分类器或加入潜空间条件 Flow。
