...
- RL 独立评估口径修复已落地：`candidate_constraints.project_requested_weather` 对动作造成的缺失天气标签执行有界、可审计投影，保留原始违规计数；`test_evaluation_summary.json.acceptance` 固定四项独立门，结构/运行失败仍不会被放宽。提交 `569eb6d` 另修复 Gym 适配层的候选 `run_dir` 证据透传；`carla-rl-04-evaluate-dev-v1_20260902_115107` 已完成并确认四项门通过，第 05 步 test 仍按独立授权单独排期。
