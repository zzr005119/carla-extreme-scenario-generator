...
## 下一步
1. 保留 12 个独立 pair 的首轮验收和 3 个原始 pair 的重复种子复验，后续统计始终按独立场景聚合，Traffic Manager 种子只作为重复测量。
2. 扩展尚未覆盖的 `generator × target_risk_level` 独立集合，优先补齐分层覆盖，而不是重复当前已完成的三个原始场景。
