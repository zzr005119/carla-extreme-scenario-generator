...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理、误差/排序稳定性诊断和反馈候选评分 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，27 个反馈短名单的 81 次外部验证计划已完成静态验收，当前等待服务器实机执行。**
...
- 反馈短名单外部验证 V1 已完成准备与静态验收：27 个独立场景各覆盖三个 Traffic Manager 种子，共 `81/81` 份配置通过 `--validate-only`；调度为 9 个随机化区组、27 个三场景小批次，生成器 × 选择通道的 9 个单元格各包含 3 个独立场景。当前尚无新增 CARLA 实测结果。
...
## 下一步
1. 在服务器执行反馈短名单外部验证的一次实机冒烟；严格验收通过后，由可恢复运行器继续完成其余配置。
2. 完成全部 81 次仿真，继续严格检查路线在途率、最大偏差、传感器完整性、CARLA 服务健康和风险元数据，不因候选来自代理模型而降低验收门槛。
