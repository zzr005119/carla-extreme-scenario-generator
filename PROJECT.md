...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理、误差/排序稳定性诊断和反馈候选评分 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，27 个反馈短名单的 81 次外部验证已严格验收 81/81。风险反馈 V2/V3、双通道配对验证、碰撞边界主动补样 V1、99 场景风险反馈 V4、50 次重复 OOF、V3/V4 同口径对比和风险分数拆解校准 V1 均已完成；当前转入物理交互派生特征与碰撞边界可预测性改进。**
...
阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条唯一 CVAE 样本实测记录、完整受控重复性验证、三生成器 108 次严格验收、第一轮 36 个独立场景风险反馈聚合、第一版风险代理基线、1536 条反馈候选的三通道统一评分，以及 27 个新增场景的 81 次外部验证；双通道评分离线比较、54 次配对实机验证、V3 风险反馈复训、1536 条候选 V3 重评分、18 个独立场景碰撞边界主动补样、V4 复训和风险分数拆解校准均已完成，当前进入物理交互派生特征改进。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测、第一版风险反馈诊断和三通道候选评分已完成；受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，反馈短名单外部验证严格验收 81/81，风险反馈 V2/V3、双通道离线评分与配对验证、V3 候选重评分、碰撞边界主动补样 V1、风险反馈 V4 和风险分数拆解校准 V1 已完成，当前进行物理交互派生特征改进。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次实机严格验收；✅ 三生成器 108 次仿真、严格验收与阶段结论冻结；✅ 风险反馈数据集 V1 与代理基线；✅ 代理误差和排序稳定性诊断 V1；✅ 反馈候选 81 次外部验证；✅ 风险反馈数据集、风险代理和碰撞通道诊断 V2；✅ 双通道离线评分与 54 次配对计划；✅ 双通道 54 次配对实机验证；✅ 风险反馈 V3 合并与复训；✅ V3 候选池重评分；✅ 碰撞边界主动补样 V1 的 54 次多传感器实机验证；✅ 风险反馈 V4、V3/V4 对比和风险分数拆解校准 V1；▶ 物理交互派生特征改进，不进行最终 Demo 封版。
...
- 当前不修改 `heuristic_v2` 权重，不通过降低碰撞权重掩盖预测误差；风险代理继续用于高风险粗筛，候选选择必须保留不确定性、多样性和碰撞边界配额。下一轮优先增加可由 15 维参数确定计算的物理交互派生特征，再做同口径离线 OOF，不立即追加 CARLA 大批次。
...
- 按原计划，2026年7—8月应处于阶段三；截至2026年8月16日，阶段二已形成第一版可验证模型基线、受控重复性证据、三生成器 108/108 严格实测对照、风险代理基线、重复 OOF 误差诊断、反馈短名单、碰撞边界主动补样和多传感器证据批次，CARLA 0.9.16 三种子迁移回归、反馈短名单 81/81 外部验证和主动补样 54/54 外部验证均已通过，风险反馈 V4 与风险分数拆解校准 V1 也已完成，当前进入物理交互派生特征改进。
...
## 下一步
1. 从现有 15 维参数构造物理交互派生特征，包括事件时间重叠、前车制动需求、行人过街窗口和天气能见度严重度，并保持所有特征可在 CARLA 运行前计算。
2. 在 V4 的 99 个独立场景上使用相同的 50 次重复分层三折 OOF，对比原始 15 维、派生特征增强和独立碰撞通道；只有碰撞子集误差或 Top-K 稳定性出现实质改善才进入候选重评分。
3. 继续保持大批量 RGB+Collision、边界补样 RGB+Depth+Semantic+Collision 的两档传感器策略，不把图像传感器强行加入全量实验。
4. 物理特征校准稳定后，再评估潜空间条件 Flow；当前不进入最终 Demo 封版。
