...
1. ▶ **S5-RL-P3.1 对抗搜索机制修复**：离线机制、`256` 步 canary、`2,000` 步 pilot 和 dev 评估已完成；pilot 训练质量门 `17/17`、CARLA 严格执行 `2,251/2,251`，两个 checkpoint 三件套连续性门全部通过；dev 两个 checkpoint 的四项工程门均为 `27/27`，`1,000` 步 checkpoint 的晋级门通过并被选中。下一步冻结该 checkpoint，设计新的独立盲测集；不直接追加 `10,000` 步。
...
## 下一步
1. P3.1 dev 晋级门已通过，冻结并记录 `1,000` 步 checkpoint；下一步设计新的独立盲测集，报告逐场景结果和不确定性，不直接追加 `10,000` 步。
