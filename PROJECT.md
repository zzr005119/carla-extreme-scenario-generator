...
- 按原计划，2026年7—8月应处于阶段三；截至2026年8月17日，阶段二已形成第一版可验证模型基线、受控重复性证据、三生成器 108/108 严格实测对照、风险代理基线、重复 OOF 误差诊断、反馈短名单、碰撞边界主动补样和多传感器证据批次，CARLA 0.9.16 三种子迁移回归、反馈短名单 81/81 外部验证、主动补样 54/54 外部验证和物理增强配对 54/54 外部验证均已通过，风险反馈 V4、风险分数拆解校准 V1、物理交互派生特征增强 V1 和 1536 候选同口径重评分也已完成，当前进行风险反馈 V5 复训与阶段二收口。
...
## 下一步
1. 将物理增强配对验证的 `18` 个独立场景合并入风险反馈 V5，保留原始 15 维与物理增强 27 维两套输入做同口径复训和重复 OOF。
2. 用 V5 检查物理特征对整体误差、碰撞子集误差、排序稳定性和候选多样性的收益是否保持；暂不修改 `heuristic_v2` 权重。
3. 若 V5 结果稳定，将物理增强代理冻结为优先候选评分分支，同时保留原始 15 维作为可复现实验基线。
4. 完成阶段二收口后进入阶段三：建设极端场景库 V1、场景元数据索引、质量评估和可检索批量管理；潜空间条件 Flow 延后到数据规模和反馈质量满足条件后再评估。
