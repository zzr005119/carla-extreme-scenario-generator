...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步/多步闭环、异常路径编排测试、Gymnasium 外壳、服务器 `check_env`、环境级 CARLA 冒烟、场景库分层采样、四类非学习离线基线、reward V2 和 60 次 CARLA 严格对照均已完成；下一步安装并验证 Stable-Baselines3，先建立低成本训练接口，不直接启动高成本 CARLA 在线训练。
...
## 下一步
1. 在服务器项目环境安装并锁定 Stable-Baselines3，验证与 Gymnasium `1.3.0`、15 维动作和 34 维观测的兼容性；先运行 `check_env` 和短步数 mock/代理训练，不直接消耗 CARLA。
2. 设计低成本训练执行器：优先复用冻结的 27 维风险代理或真实反馈数据作为训练近似环境，CARLA 只用于独立低预算验收；避免把单步 20 秒仿真直接扩成大规模在线训练。
3. 以 rule-guided LHS 为主要非学习对照，同时保留 fixed/random/LHS；策略实机评估继续使用相同 12 分层、严格验收和 reward V2，不以训练回报替代 CARLA 风险证据。
4. 单独处理服务器既有 `matplotlib` 依赖缺口；继续维护场景库质量门和 Dashboard 回归，软著演示截图后置到正式申请准备阶段。
