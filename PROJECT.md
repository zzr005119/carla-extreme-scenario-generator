...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理、误差/排序稳定性诊断和反馈候选评分 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，27 个反馈短名单的 81 次外部验证已严格验收 81/81。63 个独立场景的风险反馈数据集 V2、风险代理复训、50 次重复 OOF、同口径 Top-9 对比和独立碰撞倾向诊断均已完成，当前开始设计连续风险与碰撞倾向分离的双通道候选评分。**
...
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测、第一版风险反馈诊断和三通道候选评分已完成；受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，反馈短名单外部验证严格验收 81/81，风险反馈与代理 V2 已完成，当前构建连续风险与碰撞倾向分离的双通道反馈评分。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次实机严格验收；✅ 三生成器 108 次仿真、严格验收与阶段结论冻结；✅ 风险反馈数据集 V1 与代理基线；✅ 代理误差和排序稳定性诊断 V1；✅ 反馈候选 81 次外部验证；✅ 风险反馈数据集、风险代理和碰撞通道诊断 V2；▶ 双通道候选评分，不进行最终 Demo 封版。
...
## 下一步
1. 构建双通道候选评分 V2：连续风险通道与碰撞倾向通道分别输出预测均值、重复波动、Top-K 入选频率和多样性距离，不把两者压成同一个训练标签。
2. 在服务器生成新候选池并执行纯离线重复评分，比较单风险通道与双通道在排序稳定性、碰撞样本覆盖和生成器配额上的差异。
3. 根据离线结果形成小规模平衡验证计划；在双通道评分稳定前，不启动下一轮大规模 CARLA 仿真。
4. 双通道闭环稳定后，再决定继续碰撞边界主动补样，还是进入潜空间条件 Flow 对照。
