...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步/多步闭环、异常路径编排测试、Gymnasium 外壳、服务器 `check_env`、环境级 CARLA 冒烟、场景库分层采样和四类非学习离线基线已完成；下一步增加约束感知重采样并准备小规模 CARLA 基线对照，当前尚未训练 RL 模型。
...
## 下一步
1. 为随机、LHS 和规则引导 LHS 增加有上限的约束感知重采样，保留原始无效率与尝试次数，避免真实 CARLA 对照静默丢弃无效候选。
2. 从一个 12 分层周期准备共享基线与 fixed/random/LHS/rule-guided LHS 的小规模 CARLA 对照计划，先服务器冒烟，再决定是否执行完整严格验收。
3. 单独处理服务器既有 `matplotlib` 依赖缺口；获得真实基线奖励与运行成本后，再评估安装 Stable-Baselines3，当前仍不启动训练。
