...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。推进 OpenSCENARIO/CARLA 适配、对抗性测试代理和闭环自动化评估；当前 Demo 已提前验证其中部分 CARLA 接口和评估能力。
...
## 下一步
1. 设计对抗性测试代理的输入、动作、奖励/目标和安全终止条件，不提前宣称代理已实现。
2. 建立“生成场景 → 测试代理 → CARLA 执行 → 风险评估”的闭环实验接口。
