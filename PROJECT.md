...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步/多步闭环、异常路径编排测试、Gymnasium 外壳、服务器 `check_env` 和环境级 CARLA 冒烟已完成；下一步补充分层场景采样器和基线对照，当前尚未训练 RL 模型。
...
## 下一步
1. 增加分层场景采样器，覆盖生成器、目标风险档、天气/危险标签和随机种子，并用固定动作、随机动作、规则/LHS 建立离线基线。
2. 单独处理服务器既有 `matplotlib` 依赖缺口后，再评估是否安装 Stable-Baselines3；当前仍不启动训练。
