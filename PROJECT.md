...
## 当前阶段
**大创总体已完成阶段二“生成式 AI 模型与物理约束”的工程基线，当前进入阶段三“极端场景库与质量评估”。风险反馈 V5 已合并为 `117` 个独立场景，其中 `39` 个碰撞场景；27 维物理增强代理在 50 次重复 OOF 中取得 MAE `10.986`、Spearman `0.781`、Top-9 Jaccard `0.304`，并冻结为后续候选评分的优先实验分支，原始 15 维继续作为可复现实验基线。场景库 V1 的统一条目 Schema、来源追踪、内容哈希去重、质量字段和 CSV 检索索引已经建立，首批导入三生成器对照的 `36` 个独立场景与 `108` 次严格验收运行证据。**
...
3. ▶ **极端场景库与质量评估**：当前阶段。建立统一场景条目、来源追踪、哈希去重、真实性/多样性/危险性/可执行性质量指标和结构化检索索引，并导入已有严格验收场景。
...
- `data/scenarios/scenario_library_v1/`：首版场景库的 36 个独立场景、108 次严格验收证据和质量索引。
...
- 场景库 V1 数据契约已建立：`schemas/scenario_library_entry.schema.json` 统一记录物理参数、目标/实测标签、生成来源、文件哈希、运行证据和质量字段；`tools/build_scenario_library.py` 按物理参数内容哈希去重并生成 JSONL、CSV、汇总和清单哈希。首批从三生成器对照导入 `36` 个独立场景和 `108` 次严格验收证据，重复 `0`、排除 `0`；LHS/GMM/CVAE 各 `12` 个，目标四档各 `9` 个，实测高风险及以上 `8` 个、碰撞场景 `3` 个。
...
- 截至 2026 年 8 月 17 日，阶段二已完成并形成可复现工程基线：三生成器 108/108 严格实测对照、反馈短名单 81/81、双通道配对 54/54、碰撞边界主动补样 54/54、物理增强配对 54/54 均通过严格验收；风险反馈 V5、50 次重复 OOF 和 27 维物理增强代理冻结已完成。当前进入阶段三，先建设场景库数据契约和质量门槛，不再继续无边界扩展阶段二实验。
...
## 下一步
1. 将风险反馈 V5 的 `117` 个独立场景和严格验收来源信息接入场景库构建器，与首批 `36` 个场景按物理参数内容哈希去重。
2. 重新计算扩库后的覆盖率、最近邻多样性、风险/碰撞分布和证据完整性，区分当前 CARLA 0.9.16 证据与缺少版本字段的历史证据。
3. 建立按生成器、目标/实测风险、碰撞、CARLA 版本、质量等级和标签组合筛选的离线查询 CLI。
4. 冻结场景库 V1 的条目与查询接口后，再评估轻量可视化管理界面和软著系统模块映射。
