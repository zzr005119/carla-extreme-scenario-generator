...
1. ▶ **S5-RL-P3.1 对抗搜索机制修复**：离线机制、`256` 步 canary 和 `2,000` 步 pilot 已完成；pilot 训练质量门 `17/17`、CARLA 严格执行 `2,251/2,251`，`1,000/2,000` 两个 checkpoint 的模型/replay buffer/sampler state 三件套连续性门全部通过。下一步对两个 checkpoint 做同口径 dev 评估；dev 未同时达到平均风险增量 `> 0` 和过半场景风险上升时，不扩大训练预算。
...
- `S5-RL-P3.1` 已完成 CPU/离线机制、canary 和 pilot：V1 配置和历史证据保持冻结，新配置将动作步长降为 `0.04`、场景步数限为 `8`，评估使用 `best_so_far` 并支持目标分/停滞早停；碰撞与事件额外奖励归零，增加动作 RMS L2 和参数边界饱和惩罚。canary 作业 `carla-rl-p3-1-01-canary_20260903_135717` 以退出码 `0` 完成 `256/256` 步，质量门 `17/17`、CARLA 严格执行 `289/289`；pilot 作业 `carla-rl-p3-1-02-pilot_20260903_160538` 以退出码 `0` 完成 `2,000/2,000` 步，质量门 `17/17`、CARLA 严格执行 `2,251/2,251`，仅选择 train split，`1,000/2,000` checkpoint 三件套均完整。pilot 日志和训练轨迹仍未替代独立 dev 效果评估；dev 尚未运行，既有 P3 test 不参与 P3.1 选择或最终证明。运行协议见 `docs/carla_online_rl_p3_1.md`。
...
## 下一步
1. P3.1 `256` 步 canary 和 `2,000` 步 pilot 已通过工程质量门；下一步对 `1,000/2,000` checkpoint 做同一 dev 口径评估。晋级门未通过即停止，不直接追加 `10,000` 步。
