...
阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条唯一 CVAE 样本实测记录、完整受控重复性验证、三生成器 108 次严格验收、第一轮 36 个独立场景风险反馈聚合、第一版风险代理基线、1536 条反馈候选的三通道统一评分，以及 27 个新增场景的 81 次外部验证；双通道评分离线比较、54 次配对实机验证和 V3 风险反馈复训均已完成，下一步是使用 V3 重新评分候选池。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测、第一版风险反馈诊断和三通道候选评分已完成；受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，反馈短名单外部验证严格验收 81/81，风险反馈 V2、双通道离线评分、54 次配对验证和风险反馈 V3 复训已完成，当前比较 V3 代理对候选排序的改进。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次实机严格验收；✅ 三生成器 108 次仿真、严格验收与阶段结论冻结；✅ 风险反馈数据集 V1 与代理基线；✅ 代理误差和排序稳定性诊断 V1；✅ 反馈候选 81 次外部验证；✅ 风险反馈数据集、风险代理和碰撞通道诊断 V2；✅ 双通道离线评分与 54 次配对计划；✅ 双通道 54 次配对实机验证；✅ 风险反馈 V3 合并与复训；▶ V3 候选池重评分，不进行最终 Demo 封版。
...
## 下一步
1. 使用 V3 风险反馈数据重新生成相同规模的 LHS/GMM/CVAE 候选池并执行双通道离线评分。
2. 对比 V2/V3 代理在候选排序稳定性、碰撞倾向覆盖和生成器配额上的变化，不直接把 V3 预测分当作实测风险。
3. 若 V3 排序稳定性和碰撞边界覆盖均改善，再生成下一轮小规模配对 CARLA 计划；否则优先做碰撞边界主动补样和风险指标校准。
4. 在风险反馈闭环稳定前不进入潜空间条件 Flow，也不进行最终 Demo 封版。
