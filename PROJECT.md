...
## 下一步
1. 定义阶段四的 OpenSCENARIO/CARLA 适配边界，保留现有自定义 JSON 作为工程基线。
2. 设计对抗性测试代理的输入、动作、奖励/目标和安全终止条件，不提前宣称代理已实现。
3. 建立“生成场景 → 测试代理 → CARLA 执行 → 风险评估”的闭环实验接口。
4. 继续维护场景库质量门和 Dashboard 回归，避免阶段四实验破坏阶段三证据。
5. 软著演示截图后置到正式申请准备阶段；真实性研究继续等待同口径真实世界参数分布。
