...
## 当前阶段
**大创总体处于阶段二后段：第一版参数级生成模型与统计对照基线已完成，两轮 CVAE 抽样实测及 36 次多种子问题诊断已完成；确定性 waypoint 控制器下的 V3 小规模回归已严格验收 9/9，`cvae_repeatability_v4` 的 12 场景 × 3 种子扩展回归已严格验收 36/36。当前已完成生成器实机对照前的受控重复性基线，下一项主线工作是 LHS、GMM、CVAE 的同口径 CARLA 对照。**

当前完成的是一套 **CARLA 极端场景仿真 Demo**，它是后续生成式 AI 场景生成、物理校验和自动化测试的仿真底座，不是项目终点，也不是最终软件封版。风险指标 V2 已完成离线分析和 CARLA 实机回归。

阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条生成样本实测记录和第一轮 36 次多种子问题诊断；3 场景 × 3 种子与 12 场景 × 3 种子的受控重复性验证均已通过，但尚未完成不同生成器实机对照、风险代理和反馈引导。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM 和轻量条件表格 CVAE 基线已完成；车辆路线约束、确定性 waypoint 控制器和严格验收遥测已实现，12 场景 × 3 种子的受控重复性回归已严格验收 36/36，现进入生成器同口径实机对照准备。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次拉丁式区组实机回归与严格验收；▶ 准备 LHS/GMM/CVAE 同口径实机对照，不进行最终 Demo 封版。
...
- `configs/`：主场景和批次配置。
- `configs/route_control_profiles/waypoint_follower_v1.json`：已通过 V3 实机验证的冻结控制器、传感器和严格验收配置。
...
- `data/scenarios/cvae_repeatability_v4/`：12 场景 × 3 种子的拉丁式区组配置、9 个分批脚本、严格验收清单和自动汇总入口。
...
## 技术栈
- **当前使用**：Python 3.10、CARLA 0.9.15、NumPy 2.2.6、PyTorch 2.12.1+cu132、Pillow、JSON、CSV；`Carla666` 环境已确认 CUDA 可用，GPU 为 NVIDIA GeForce RTX 4060 Laptop GPU。
...
- `seed_v1` 的风险等级和模型输出中的 `target_risk_level` 均是参数设计目标；目前只有 16 条 CVAE 样本具有 CARLA 实测 `observed_risk`，其余样本不能视为真实风险标签。离线有效率也不能替代 Actor 生成、场景完成和风险评分的实机证据。
- 当前 CVAE 主要学习人工设计的参数区间和相关关系，训练样本仅 `256` 条；它是参数生成与闭环接口基线，不代表已学习真实交通分布。
- 当前离线结果显示规则/LHS 在设计区间一致性上仍强于 CVAE；模型价值需要通过 CARLA 成功率、实测风险命中率和后续风险反馈搜索证明。
...
- V4 高风险目标 9 次均稳定为 `medium`，临界目标也未达到 `critical`。不能通过弱化控制器或直接修改标签来提高命中率；应在同一控制器下完成 LHS/GMM/CVAE 实测对照，再判断调整生成参数搜索还是审查风险指标。
- 按原计划，2026年7—8月应处于阶段三；截至2026年8月14日，阶段二已形成第一版可验证模型基线、16 条唯一 CVAE 样本的实测记录和完整受控重复性证据，但生成器实机对照仍未完成，进度需通过下一轮 LHS/GMM/CVAE 对照追回。
- 服务器 `gpu03` 暂不可用；当前在 RTX 4060 笔记本执行 CARLA 调试与批次实验。
- 长时间多传感器运行仍可能产生硬件压力，但帧完整性和服务健康检查已作为批次验收条件。
- 当前代码改动尚未统一 Git 提交；`项目进度文档.md` 已由用户主动删除，后续以 `PROJECT.md` 维护项目当前状态。

## 下一步
1. 固定 `waypoint_follower_v1`、三个交通种子和严格验收口径，设计 LHS、GMM、CVAE 的等规模 CARLA 实测对照；优先采用每种生成器 12 个场景、每档 3 个、每场景 3 个种子的平衡方案。
2. 先完成三种生成器候选样本的离线筛选、去重、配置编译、控制配置哈希固化和 108 次运行计划静态校验，再进入 CARLA 分批运行。
3. 实机对照完成后比较 CARLA 成功率、连续风险分布、目标风险命中率、场景多样性和高风险样本产出率，确定第一版场景生成器主线；报告中继续分离 `target_risk_level` 与 `observed_risk`。
4. 基于同口径对照结果审查 `heuristic_v2` 的 critical 定义，重点判断“critical 是否必须发生碰撞”是否符合项目的极端场景筛选目标；在证据形成前不直接修改阈值。
5. 建立可重复追加的实测数据集和训练数据版本管理；随后训练风险代理并比较反馈搜索、候选排序和潜变量优化。
6. 当成功实测记录和高风险样本达到调研报告中的切换门槛后，再加入潜空间条件 Flow；不在当前数据条件下直接实现完整轨迹扩散或世界模型。
