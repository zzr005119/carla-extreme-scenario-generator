...
- RL 独立评估口径修复已落地：`candidate_constraints.project_requested_weather` 对动作造成的缺失天气标签执行有界、可审计投影，保留原始违规计数；`test_evaluation_summary.json.acceptance` 固定四项独立门，结构/运行失败仍不会被放宽。新提交需先重跑 dev，再允许第 05 步 test。
