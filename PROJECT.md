...
阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条唯一 CVAE 样本实测记录、完整受控重复性验证、三生成器 108 次严格验收、第一轮 36 个独立场景风险反馈聚合、第一版风险代理基线、1536 条反馈候选的三通道统一评分，以及 27 个新增场景的 81 次外部验证；双通道评分离线比较、54 次配对实机验证、V3 风险反馈复训和 1536 条候选 V3 重评分均已完成，当前设计碰撞边界主动补样。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测、第一版风险反馈诊断和三通道候选评分已完成；受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，反馈短名单外部验证严格验收 81/81，风险反馈 V2、双通道离线评分、54 次配对验证、风险反馈 V3 复训和 V3 候选重评分已完成，当前进行碰撞边界主动补样与指标校准。
...
5. ✅ V2 实机回归；✅ 第一版生成模型离线基线；✅ 两轮 CVAE 生成样本抽样验证；✅ 第一轮 36 次多种子问题诊断；✅ 路线失败诊断；✅ 确定性 waypoint 控制器；✅ V3 九次实机严格验收；✅ V4 三十六次实机严格验收；✅ 三生成器 108 次仿真、严格验收与阶段结论冻结；✅ 风险反馈数据集 V1 与代理基线；✅ 代理误差和排序稳定性诊断 V1；✅ 反馈候选 81 次外部验证；✅ 风险反馈数据集、风险代理和碰撞通道诊断 V2；✅ 双通道离线评分与 54 次配对计划；✅ 双通道 54 次配对实机验证；✅ 风险反馈 V3 合并与复训；✅ V3 候选池重评分；▶ 碰撞边界主动补样与风险指标校准，不进行最终 Demo 封版。
...
- 服务器生成的本轮结果目录使用 `20260816` 时间戳；项目当前日期为 2026-08-15，因此该目录名按服务器时钟超前处理，不作为实验完成日期依据。
...
## 下一步
1. 基于 V3 候选池定义碰撞边界主动补样规则，优先覆盖预测碰撞倾向高但连续风险不稳定的样本。
2. 审查 `heuristic_v2` 的碰撞与连续风险合成关系，保持 `target_risk_level` 与 `observed_risk` 分离，不通过修改标签提高命中率。
3. 对主动补样平衡小批次使用多传感器证据配置，对其余批次继续使用 RGB-only 基线，并保留路线、传感器和服务健康质量门槛。
4. 碰撞边界数据稳定后，再评估潜空间条件 Flow；当前不进入最终 Demo 封版。
