...
## 下一步
1. 将场景生成、场景库管理、仿真执行和风险评估映射为软著系统模块，确定模块边界和最小可运行闭环。
2. 把质量门和质量分析纳入每次扩库后的验收流程，比较风险分布、证据等级、目标偏差和多样性离群变化。
3. 评估轻量可视化管理界面的最小功能范围，优先覆盖场景筛选、详情、运行证据和风险结果展示。
4. 调研可映射到 15 维参数的公开或真实世界数据来源；获得同口径参考分布前，真实性继续保持 `not_assessed`。
