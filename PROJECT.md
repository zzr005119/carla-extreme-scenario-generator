...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理、误差/排序稳定性诊断和反馈候选评分 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，当前进入反馈短名单的 CARLA 外部验证。**
...
## 下一步
1. 将 27 个反馈短名单编译为冻结 `waypoint_follower_v1` 的 CARLA 0.9.16 外部验证清单；每个场景使用三个 Traffic Manager 种子重复运行，共计划 `81` 次，并按生成器、选择通道和目标档做平衡调度。
2. 在服务器分批执行 81 次仿真，继续严格检查路线在途率、最大偏差、传感器完整性、CARLA 服务健康和风险元数据，不因候选来自代理模型而降低验收门槛。
3. 聚合 27 个新增独立场景，分别评估稳健分与实测风险的排序误差、三个通道的碰撞/高风险发现率，以及新增样本加入后对风险代理误差和 Top-K 稳定性的修正作用。
4. 当成功实测记录和高风险/碰撞样本达到调研报告中的切换门槛后，再训练独立碰撞分类器或加入潜空间条件 Flow；当前不实现完整轨迹扩散或世界模型。
