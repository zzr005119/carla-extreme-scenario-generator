...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步/多步闭环、异常路径编排测试、Gymnasium 外壳、环境级 CARLA 冒烟、场景库分层采样、四类非学习基线、reward V2、60 次 CARLA 严格对照、Stable-Baselines3 训练工程链路以及冻结 27 维风险代理执行器均已完成；下一步进行多随机种子 SAC/PPO 代理训练与等预算非学习基线比较，再冻结少量 CARLA 独立评估计划，不直接启动 CARLA 在线训练。
...
## 下一步
1. 在冻结代理执行器上进行多随机种子 SAC/PPO 低成本训练与稳定性检查，并以相同候选预算对照 fixed/random/LHS/rule-guided LHS；所得结论只属于代理环境。
2. 冻结少量 12 分层 CARLA 独立评估计划：策略推理后不在线更新，继续使用共享基线、严格验收和 reward V2；以 rule-guided LHS 为主要非学习对照，同时保留其余三类基线。
3. CARLA 独立评估恢复风险、碰撞和安全事件三个 reward V2 通道；代理训练期间不预测、伪造或奖励未建模的碰撞/事件信号。
