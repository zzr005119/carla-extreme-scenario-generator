...
4. ▶ **仿真平台与对抗性测试代理**：当前阶段。OpenSCENARIO/CARLA 最小适配、对抗性测试代理 V1 契约、单步闭环和 3 步真实反馈闭环冒烟已完成；下一步补充失败中止与重复截断编排测试，当前尚未训练 RL 模型。
...
## 下一步
1. 补充多步 candidate 失败中止、无效候选恢复和重复场景截断的编排级测试，并保留失败原因与终止状态证据。
2. 完成上述状态机测试后，再评估是否引入 `gymnasium` 与 Stable-Baselines3；在训练依赖和策略基线形成前，不宣称 RL 训练完成。
