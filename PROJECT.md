...
## 当前阶段
**大创总体处于阶段二收尾：第一版参数级生成模型、确定性 waypoint 控制器、统计对照基线、实测风险代理和误差/排序稳定性诊断 V1 已完成；`cvae_repeatability_v4` 已严格验收 36/36，LHS、GMM、CVAE 同口径 CARLA 对照 V1 已严格验收 108/108，当前进入反馈引导候选排序与碰撞边界主动补样。**
...
阶段一中的 CARLA 环境、场景参数化、多传感器采集和批量验证已完成；生成式 AI 相关文献已完成第一轮收集和模型选型调研（56 篇索引、46 篇本地 PDF），公开数据集调研仍需补齐。阶段二已完成 15 维统一表示、种子数据集、条件 GMM、轻量条件表格 CVAE、多随机种子训练、离线评估、16 条唯一 CVAE 样本实测记录、完整受控重复性验证、三生成器 108 次严格验收、36 个独立场景风险反馈聚合和第一版风险代理基线。

## 整体阶段顺序
1. ✅/▶ **基础调研与环境搭建**：CARLA、Python、Git/GitHub 和仿真底座已完成；生成式 AI 文献第一轮收集和模型选型已完成，公开数据集收集与预处理仍需补齐。
2. ▶ **生成式 AI 模型与物理约束**：当前主线。场景 Schema、独立校验器、种子数据集 V1、规则/LHS、条件 GMM、轻量条件表格 CVAE、确定性 waypoint 控制器、严格验收遥测和第一版风险反馈诊断已完成；12 场景 × 3 种子的受控重复性回归严格验收 36/36，三生成器同口径实机对照严格验收 108/108，当前构建稳健候选排序与主动补样策略。
...
- 按原计划，2026年7—8月应处于阶段三；截至2026年8月15日，阶段二已形成第一版可验证模型基线、受控重复性证据、三生成器 108/108 严格实测对照、风险代理基线和重复 OOF 误差诊断，CARLA 0.9.16 三种子迁移回归已通过，当前进入反馈候选排序和外部验证设计。
...
## 下一步
1. 实现反馈候选评分 V1：输出重复模型预测均值、预测标准差、稳健排序分、Top-K 入选频率和参数空间多样性，不能只使用单次代理分数。
2. 在服务器分别生成 LHS、GMM 和 CVAE 候选池；每种生成器同时选择稳定高分候选、高不确定性候选和碰撞边界邻域候选，避免三种策略使用不同验收口径。
3. 使用冻结控制器和 CARLA 0.9.16 做小规模外部验证，分别评估连续风险排序误差、碰撞发现率和新增样本对代理模型的修正作用；不把当前 36 个场景当作最终统计结论。
