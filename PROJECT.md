...
## 下一步
1. 完成环境级服务器 CARLA 冒烟，确认真实 executor 的 `reset/step` 与终止状态回填。
2. 处理或单独登记服务器既有 `matplotlib` 依赖缺口后，再设计固定动作、随机动作、规则/LHS 和 SAC/PPO 小规模对照；当前仍不启动训练。
